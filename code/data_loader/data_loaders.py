...
from constant import RELATION_VISUAL_MASK
...
class RelationVisualMaskDataLoader(BaseDataLoader):
    def batch_collate(data):
        titles_ids = []
        features = []
        matches = []
        ...
        return (
            (titles_ids.long(), features.float(), titles_mask.bool(), visual_masks.bool()),
            (global_masks.bool(), attr_masks.bool(), matches.float())
        )
    ...
    class InnerDataset(Dataset):
        ...
        def __getitem__(self, index):
            ...
            # visual mask
            if self.mask_global:
                visual_mask = np.array([0] * 12)
                for attr in data['key_attr'].keys():
                    visual_mask = visual_mask | RELATION_VISUAL_MASK[attr_id_map[attr]]
                visual_mask = list(visual_mask)
                visual_mask.append(1)
            ...
            if data['is_fine'] or self.training:
                for k, text in data['key_attr'].items():
                    ...
                    # visual mask
                    if self.mask_attr:
                        visual_mask = list(RELATION_VISUAL_MASK[attr_id_map[attr]])
...
class VisualMaskDataLoader2(BaseDataLoader):
    def batch_collate(data):
        texts_ids = []
        features = []
        matches = []
        ...
        for d in data:
            texts_ids.append(torch.tensor(d['text_ids']).long())
            features.append(d['feature'])
            matches.append(d['match'])
            loss_masks.append(d['loss_mask'])
            visual_masks.append(d['visual_mask'])
            loss_weight = d['loss_weight']

        texts_ids = nn.utils.rnn.pad_sequence(
            texts_ids, batch_first=True, padding_value=0)
        texts_mask = (texts_ids != 0).long()
        features = torch.tensor(features)
        matches = torch.tensor(matches)
        ...
        return (
            (texts_ids.long(), features.float(), texts_mask.bool(), visual_masks.bool()),
            (matches.float(), loss_masks.bool(), loss_weight.float())
        )
    ...
    class InnerDataset(Dataset):
        def __init__(self, 
            ...
            if drop_no_attr:
                print('=== run drop_no_attr ===')
                ori_len = len(self.data)
                self.data = [d for d in self.data if len(d['key_attr']) > 0]
                print(f'ori_len: {ori_len}, new_len: {len(self.data)}')
        ...
        def __getitem__(self, index):
            ...
            for attr in attrs:
                ...
                match[attr_id_map[attr]] = 1

            # generate neg sample
            if match[-1]:
                loss_mask = visual_mask
                self.attr_match1 += len(attrs)

                if len(attrs) >= 1 and given_prob(self.global_match0_prob):
                    match[-1] = 0
                    # replace attrval
                    replace_attr = random.choice(attrs)
                    replace_attrval = data['key_attr'][replace_attr]
                    text = self.replace_attr(text, replace_attr, replace_attrval)

                    match[attr_id_map[replace_attr]] = 0
            ...
            # update global match rate
            self.global_match0 += (match[-1] == 0)
            self.global_match1 += (match[-1] == 1)
            ...
            ret = {
                ...
                'loss_weight': [
                    (self.attr_match0 + self.attr_match1) / (2 * self.attr_match0),
                    (self.attr_match0 + self.attr_match1) / (2 * self.attr_match1)
                ]
            }
...
class VisualMaskDataLoader(BaseDataLoader):
    ...
    def __init__(self, raw_data_dir, tokenizer_path, length=-1, run_jieba=False,
                mask_attr=True, mask_global=False, drop_no_attr=False,
                batch_size=128, shuffle=True, fold_idx=-1, validation_split=0.25, num_workers=1, training=True):
        self.dataset = self.__class__.InnerDataset(
            raw_data_dir=raw_data_dir, 
            tokenizer_path=tokenizer_path, 
